...
### 5.1 テンパイ判定の実装
- [ ] TenpaiAnalyzer モジュールの作成
- [ ] isTenpai関数の実装（13牌からテンパイ判定）
- [ ] 待ち牌リストの取得関数
- [ ] テンパイ判定のテストケース実装
...
- [ ] テンパイチェック → 点数チェック → ゲーム段階チェックのResult型チェーン
- [ ] 複数バリデーションエラーの適切な表現
- [ ] bind (>>=) 演算子による関数合成
- [ ] パラメーター注入による外部状態管理

### 5.4 エラー集約パターンの実装
- [ ] Result<'T, 'Error list> 型によるエラー集約
- [ ] 並列バリデーションの実装（fail-fast vs すべてのエラー収集）
- [ ] Applicative スタイルでの複数バリデーション合成
- [ ] エラー集約ユーティリティ関数の実装（sequence、traverse等）
- [ ] 集約されたエラーの適切な表示

### 5.5 テストケース実装
- [ ] 成功ケース（正常なリーチ宣言）
- [ ] 失敗ケース（テンパイしていない、点数不足、ゲーム終盤など）
- [ ] エラーパイプラインの動作確認
- [ ] エラー集約パターンのテスト（複数エラーの同時発生）
