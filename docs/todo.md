...
- [ ] 以下のモジュールを移動：
  ...
  - [ ] TenpaiAnalyzer.fs（テンパイ分析サービス）
  - [ ] YakuAnalyzer.fs（役判定サービス）
- [ ] Domainプロジェクトへの参照設定
- [ ] 既存テストの移行と動作確認

### 6.3 Workflowsの分離
- [ ] FunctionalDddMahjong.Workflowsプロジェクト作成
- [ ] Reach.fsをReachWorkflow.fsとして移動
- [ ] ワークフロー層の責務確認
- [ ] 統合テストの実装

### 6.4 プロジェクト構造の最終確認
- [ ] 依存関係の方向性確認（内側に向かう依存）
- [ ] 各レイヤーの責務が明確に分離されているか検証
- [ ] README.mdの更新（新しいプロジェクト構造の反映）

**習得スキル**
- [ ] **関数型アーキテクチャ**: Clean Architecture/Onion Architectureの関数型版
