...
module YakuAnalyzer =
    ...
    let private mutuallyExclusivePairs =
        [
          ...
          Set.ofList [ Iipeikou; Toitoi ]
          // 将来の例: 二盃口とトイトイも相互排他
          // Set.ofList [ Yaku.Ryanpeikou; Yaku.Toitoi ]
          ]

    // 相互排他関係を解決して最終的な役リストを取得
    let private resolveExclusivity (allDetectedYaku: Yaku list) : Yaku list =
        ...
        let resolvedYaku =
            ...
            |> List.fold
                (fun currentYaku exclusivePair ->
                    ...
                    | multiple ->
                        // 複数ある場合は最高翻数の役を選択
                        let bestYaku = multiple |> List.maxBy getHan
    ...
    let validateWinningHand (hand: Hand.Hand) : Result<Hand.WinningHand, YakuError> =
        if Hand.isWinningHand hand then
            match Hand.tryCreateWinningHand hand with
            | Some winningHand -> Ok winningHand
            | None -> Error(YakuError.InvalidHandState "和了形の手牌を作成できませんでした")
        else
            Error(YakuError.InvalidHandState "和了していない手牌です")
    ...
    let analyzeAllYaku (winningHand: Hand.WinningHand) : YakuAnalysisResult =
        ...
        if List.isEmpty allDetectedYaku then
            ...
            let totalHan =
                finalYaku |> List.map getHan |> List.sum

            { Yaku = finalYaku
              TotalHan = totalHan }

    // メイン分析関数（Railway-Oriented Programming）
    let analyzeYaku (hand: Hand.Hand) : Result<YakuAnalysisResult, YakuError> =
        validateWinningHand hand
        |> Result.map analyzeAllYaku

    // 役なしの場合の判定
    let hasNoYaku (result: YakuAnalysisResult) : bool = List.isEmpty result.Yaku
